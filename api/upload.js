...
// Parseia CSV — detecta separador (vírgula ou ponto-e-vírgula) automaticamente
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Valida nome: aaaa.csv (anual) ou mm_aaaa.csv (mensal)
    const matchAnual = filename.match(/^(\d{4})\.(csv|xls|xlsx)$/i);
    const matchMensal = filename.match(/^(\d{2})_(\d{4})\.(csv|xls|xlsx)$/i);
    if (!matchAnual && !matchMensal) {
      return res.status(400).json({ error: 'Nome inválido. Use aaaa.csv (ex: 2026.csv) ou mm_aaaa.csv (ex: 01_2026.csv)' });
    }
    const isAnual = !!matchAnual;
    const ano = isAnual ? matchAnual[1] : matchMensal[2];
    const mes = isAnual ? null : matchMensal[1];
    const key = isAnual ? `dados:${ano}` : `dados:${ano}:${mes}`;

    // Parseia conteúdo
    let rows = [];
    if (contentType === 'csv' || filename.toLowerCase().endsWith('.csv')) {
      // content é texto CSV em base64 ou direto
      // Tenta UTF-8 primeiro, depois latin-1 (Excel PT-BR)
      const raw = Buffer.from(content.startsWith('data:') ? content.split(',')[1] : content, 'base64');
      let text;
      try {
        text = raw.toString('utf-8');
        // Verifica se há caracteres inválidos (sinal de encoding errado)
    ...
    // Atualiza índice de arquivos disponíveis
    ...
    // Entry: "2026" para anual, "2026-01" para mensal
    const entry = isAnual ? ano : `${ano}-${mes}`;
    ...
    return res.status(200).json({
      ...
      key,
      message: `${processed.length} registros importados para ${isAnual ? ano : mes+'/'+ano}`
    });
